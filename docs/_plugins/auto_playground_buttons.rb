...
module Jekyll
  module AutoPlaygroundButtons
    ...
    Jekyll::Hooks.register [:pages, :documents], :post_render do |doc|
      ...
      # Skip if not a markdown-generated page
      next unless doc.respond_to?(:relative_path) && doc.relative_path.to_s.end_with?('.md')
      ...
      # Find all code blocks - need to check multiple patterns since syntax_highlighter is disabled
      code_blocks = html_doc.css('code')
      ...
      code_blocks.each do |code_block|
        ...
        # Check if this is an ISL code block by:
        # 1. Class name contains 'isl'
        # 2. Parent pre has data-lang attribute
        # 3. Code content looks like ISL (contains $ variables, fun keyword, etc.)
        code_classes = code_block['class'].to_s.downcase
        parent_classes = parent_pre['class'].to_s.downcase
        data_lang = parent_pre['data-lang'].to_s.downcase
        
        is_isl = code_classes.include?('isl') || 
                 parent_classes.include?('isl') ||
                 data_lang == 'isl'
        
        # If no class indicator, try to detect by content (look for ISL patterns)
        unless is_isl
          code_text = code_block.text.strip
          # Look for ISL-specific patterns
          is_isl = code_text.match?(/\$input/) || 
                   code_text.match?(/fun\s+\w+\s*\(/) ||
                   code_text.match?(/\|\s*\w+/) ||  # pipe modifiers
                   code_text.match?(/foreach\s+\$/)
        end
...
<div class="playground-button-container">
  <a href="#{playground_url}?isl_encoded=#{isl_encoded}&input_encoded=#{input_encoded}" 
    ...
    ▶️ Run in Playground
      ...
      # Log results
      if buttons_added > 0
        Jekyll.logger.info "AutoPlaygroundButtons:", "Added #{buttons_added} button(s) to #{doc.relative_path}"
      end
