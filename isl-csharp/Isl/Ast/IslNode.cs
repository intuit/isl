...
public record FunctionDecl(string Name, List<string> Parameters, List<Statement> Body) : IslNode;

// ---- Statements ----
public abstract record Statement : IslNode;

// $var = value  or  $var: value
public record AssignVariable(string Name, Expr Value) : Statement;
...
public record SwitchStatement(Expr Subject, List<SwitchCase> Cases, List<Statement>? ElseBody) : Statement;
...
public record PropAssign(List<string> Path, Expr Value) : ObjectProperty;
public record TextPropAssign(string Key, Expr Value) : ObjectProperty;
